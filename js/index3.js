...
function init() {
  ...
  this.controller.on('gesture',function(gesture){
    switch (gesture.type){
      case "keyTap":
      alert("Screen Tap gesture "+gesture.direction);
    }
  });
    // controller.use('screenPosition',{positioning: 'relative'});
    // controller.use('riggedHand');
    // controller.connect();
...
var loadOBJ = function() {
  ...
  loader.load('http://mamboleoo.be/learnThree/demos/banana.obj', addBananaInScene);
    loader.load('http://mamboleoo.be/learnThree/demos/banana.obj', addBananaInScene2);
};
...
var addBananaInScene2 = function(object) {
  banana = object;
  //Move the banana in the scene
  banana.rotation.x = Math.PI /2;
  banana.position.y = -255;
  banana.position.z = 90;
  banana.rotation.z=Math.PI/4;
  //Go through all children of the loaded object and search for a Mesh
  object.traverse(function(child) {
